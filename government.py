...
class Government:
    ...
    def get_profit_tax(self, firms):
        for firm in firms:
            ...
            self.history.add_record({'step': firm.step, 'participant_id': firm.id, 'action': 'profit_tax', 'money': profit_tax})
    ...
    def provide_help(self, households):
        for household in households:
            if household.employer is None:
                ...
                self.history.add_record({'step': household.step, 'participant_id': household.id, 'action': 'help', 'money': help})
