...
class GovernmentHistory:
    def __init__(self, output = "government_output.csv"):
        self.output = output
        for variable in ['step', 'participant_id', 'action', 'money', 'tax']:
        ...
        try:
            ...
            writer = csv.DictWriter(output_file, dialect = 'excel',
                                    fieldnames=['step', 'participant_id', 'action', 'money', 'tax'])
            writer.writeheader()
            output_file.close()
    ...
    def add_record(self, record):
        ...
        with open(self.output, "a", newline='') as output_file:
            writer = csv.DictWriter(output_file, dialect='excel', fieldnames=['step', 'participant_id', 'action', 'money', 'tax'])
