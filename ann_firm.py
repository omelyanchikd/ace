...
class AnnFirm(DecisionMaker):
    def __init__(self, id, firm, learning_data):
        ...
        self.neural_network = learning_data['neural_network']
        for parameter in ['world_salary', 'world_price', 'world_sold', 'world_sales']:
            setattr(self, 'neural_network_' + parameter, learning_data['neural_network_' + parameter])
    ...
    def generate_parameters(self, firm):
        ...
        for parameter in self.external[3:]:
            trial_parameters.append(getattr(self, 'neural_network_' + parameter).predict(numpy.array(trial_parameters[:3]).reshape(1, -1)))
    ...
    def decide_salary(self, stats, firm):
        ...
        for i in range(0, 100):
            ...
            has_profit = self.neural_network.predict(numpy.array(trial_parameters).reshape(1, -1))
            if has_profit == 1:
